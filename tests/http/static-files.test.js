...
describe('static files', () => {
  ...
  Object.keys(files).forEach((url) => {
    it(`should return okay when accessing ${url}`, async(done) => {
      let response = await instance.inject({ url });
      let { type, cache, vary } = files[url];

      should(response.statusCode).equal(200);
      should(response.headers['content-type']).equal(type);
      should(response.headers['cache-control']).equal(cache);
      should(response.headers.vary).equal(vary);
